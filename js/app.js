...
// Load media (supports dataUrl stored directly in DB)
...
onValue(mediaRef, (snapshot) => {
  ...
  Object.entries(data).forEach(([id, item]) => {
    ...
    if (item.type === 'image') {
      div.innerHTML = `${title}<img src="${src}" alt="${escapeHtml(item.title||'')}" style="max-width:100%;height:auto;border-radius:6px"/>`;
    } else if (item.type === 'audio') {
      div.innerHTML = `${title}<audio controls src="${src}" style="width:100%"></audio>`;
    } else if (item.type === 'video') {
      div.innerHTML = `${title}<video controls src="${src}" style="width:100%"></video>`;
    } else {
      div.innerHTML = `${title}<a href="${src}" target="_blank">Open</a>`;
    }
