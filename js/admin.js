import { app, db, EMAIL_FUNCTION_URL } from './firebase-init.js';
import { ref, push, set, onValue, get } from "https://www.gstatic.com/firebasejs/9.23.0/firebase-database.js";
...
// Upload media (store as data URL in Realtime DB)
function setupMediaForm(){
  ...
  mediaForm.addEventListener('submit', async (e) => {
    ...
    // Read file as data URL and store directly in Realtime Database
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        mediaMsg.textContent = 'Upload saved to database.';
        mediaForm.reset();
        setTimeout(()=>mediaMsg.textContent='','3000');
      } catch (err) {
        console.error(err);
        mediaMsg.textContent = 'Failed to save media: '+err.message;
      }
    };
    reader.onerror = () => {
      mediaMsg.textContent = 'Failed to read file.';
    };
